...
import 'package:anti_facebook_app/UserData/user_data.dart';
import 'package:anti_facebook_app/features/notifications/widgets/single_notification.dart';
import 'package:anti_facebook_app/models/noti.dart';
  ...
  void initState() {
    ...
    UserData.getStringData('token').then((token) {
      ...
      RequestAPI.postRequest(
          'https://it4788.catan.io.vn/get_notification', 200, data, token)
          .then((responseData) {
        if (responseData != null) {
          Map<String, dynamic> dataJs = json.decode(responseData);
          print(dataJs['data']);
          for (var notification in dataJs['data']) {
            String content = '',
                type = '',
                bold = '',
                time = '';
            DateTime pastTime = DateTime.parse(notification['created']);
            DateTime now = DateTime.now();
            // Duration sevenHours = Duration(hours: 7);
            // now = now.subtract(sevenHours);
            Duration diffTime = now.difference(pastTime);
            print(diffTime.inHours);
            if (diffTime.inMinutes < 1) {
              time = 'Vừa xong';
            } else if (diffTime.inHours < 1) {
              time = '${diffTime.inMinutes} phút';
            } else if (diffTime.inHours < 24) {
              time = '${diffTime.inHours} giờ';
            } else {
              time = '${diffTime.inHours ~/ 24} ngày';
            }
            if (notification['type'] == '1') {
              content = notification['user']['username'] +
                  ' đã gửi cho bạn lời mời kết bạn';
              type = 'friend';
              bold = notification['user']['username'];
            } else if (notification['type'] == '5') {
              content = notification['user']['username'] + ' đã thích bài viết của bạn';
              type = 'like';
              bold = notification['user']['username'];
            } else if (notification['type'] == '2') {
              content = notification['user']['username'] + ' đã chấp nhận lời kết bạn của bạn';
              type = 'accept';
              bold = notification['user']['username'];
            }
            Noti noti = Noti(
              ...
              content: content,
              image: notification['avatar'].isEmpty
                  ? 'assets/images/default_avatar.png'
                  : notification['avatar'],
              time: time,
              type: type,
              bold: [bold],
              seen: notification['read'] == '1' ? true : false,
            );
            setState(() {
